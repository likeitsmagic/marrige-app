...
export const Entry = () => {
    ...
    if (isLoading) {
        return <Flex height="100vh" width="100%" alignItems="center" justifyContent="center">
            <Spin />
        </Flex>
    }
	...
	return <PageLayout compact={false}>
    <React.Fragment>
    <style>
            {`.g-root {
    ...
    <PageLayoutAside
        headerDecoration
        logo={{
            text: () => <Text variant="header-1">{i18n.i18n("application", "title")}</Text>,
            iconSrc: logoIcon,
            iconSize: 42,
            wrapper: (node) => <Flex alignItems="center" justifyContent="center">{node}</Flex>,
            onClick: () => navigate('/')
        }}
        menuItems={[
            {
                id: "my-company",
                title: <Text variant="body-2">{i18n.i18n("application", "pages_my_business")}</Text>,
                icon: House,
                onItemClick: () => navigate('/my-business')
            },
            {
                id: "messages",
                title: <Text variant="body-2">{i18n.i18n("application", "pages_messages")}</Text>,
                icon: Comment,
                onItemClick: () => navigate('/messages')
            },
            {
                id: "statistics",
                title: <Text variant="body-2">{i18n.i18n("application", "pages_statistics")}</Text>,
