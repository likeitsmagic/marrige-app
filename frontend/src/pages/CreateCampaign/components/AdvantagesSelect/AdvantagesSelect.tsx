...
export const AdvantagesSelect = () => {
    ...
    const handleChange = useCallback(async (id: string) => {
        const newSelectedAdvantages = value.includes(id) ? value.filter((item) => item !== id) : [...value, id];
        await setValue(newSelectedAdvantages);
    }, [setValue, value]);


	return <CheckboxGroup name="advantages" label={t("advantages")} orientation="horizontal">
    {advantages?.data?.map((advantage) => (
        <CustomCheckbox key={advantage.id} value={advantage.id} onClick={() => handleChange(advantage.id)} disabled={isSubmitting}>
            {advantage.textRu}
        </CustomCheckbox>
    ))}
</CheckboxGroup>;
